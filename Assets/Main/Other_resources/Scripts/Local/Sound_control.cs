...
public class Sound_control : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    void Awake()
    {
        if (Type == Type_sound.Music)
            Setting_administrator_Sound = Setting_PlayerPrefs.Know_parameter_value(Type_parameter_value.Sound_music);
        else if (Type == Type_sound.Effect)
            Setting_administrator_Sound = Setting_PlayerPrefs.Know_parameter_value(Type_parameter_value.Sound_effect);
